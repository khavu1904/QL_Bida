...
namespace QUANLIBIDA
{
    public partial class frmNhapHang : Form
    {
        ...
        private void frmNhapHang_Load(object sender, EventArgs e)
        {
            ...
            cbbNhanVienNhap.DataSource = db.NhanViens.Where(x => x.isDeleted == 0);
            cbbNhanVienNhap.DisplayMember = "HoVaTen";
            cbbNhanVienNhap.ValueMember = "UserName";
            cbbNhanVienNhap.SelectedIndex = -1;

            cbbNhaCungCap.DataSource = db.NhaCungCaps.Where(x => x.isDeleted == 0);
        ...
        private void ShowData()//hàm hiển thị danh sách các hóa đơn nhập hàng
        {
            ...
            var rs = from o in db.HoaDonNhaps
                     ...
                     select new
                     {
                         ...
                         tongtien = db.ChiTietHoaDonNhaps.Where(x => x.IDHoaDon == o.ID).Sum(y => y.SoLuong*y.DonGia),//tổng tiền được tính dựa vào tổng các mặt hàng: sl*dg tương ứng với hóa đơn, 
        ...
        private void dgvHoaDonNhap_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                new frmChiTietHoaDonNhap(int.Parse(r.Cells["id"].Value.ToString()), danhapkho).ShowDialog();//truyền  2 tham số vào frmChiTietHoaDonNhap rồi show lên
                ShowData();//gọi lại hàm hiển thị sau khi phiếu nhập đc thêm.xóa mặt hàng
            }
        }
    }
